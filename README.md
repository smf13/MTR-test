...
Everything else (retention days, route change memory, reverse DNS, ASN lookup, notification channels, public URL, tag colours, the Globalping token, the ip-api.com switch, the MaxMind credentials) is set in the UI under **Settings** and stored in the database.
...
4. Hop IPs are reverse-resolved (cached), the route signature is compared with the recent runs that reached the destination (an unreachable run is padded with unknown hops, so comparing across an outage would report a bogus reroute), thresholds are evaluated, and the run and its hops are written in one transaction. A hop shown as `???` is a wildcard in that comparison, in the route timeline and in the path summary: many routers rate-limit or deprioritise the ICMP replies mtr relies on, so a silent hop is treated as loss at that position, never as a reroute or a new address.
5. A path run is a route change only when its hop sequence (unknown hops are wildcards) matches none of the last **Route change memory** runs that reached the destination (Settings, default 20). A load-balanced path flapping between the same few routes therefore stays quiet, while a route not seen in that window is reported; set the memory to 1 to compare with the previous run alone.
