...
- **Dashboard and detail navigation**: Dashboard shows health summaries, filters and target cards/table before the optional comparison chart. `TargetDetail` uses `Tabs` for Overview, Path analysis, Runs and Events. Path analysis is omitted for non-path probes, and a remembered path view falls back to Overview for those targets. Its nested tabs are Current path, Path history and Summary. Overview contains the distribution/hourly heatmap and non-path check details; Path analysis contains the path profile.
...
- **Hop tables and legends**: `HopTable` defaults to Compact and persists the column choice under `mtr-tracker.hop-columns`; a `compact` prop forces the reduced view. Sticky identity columns and headers are styled in `index.css`. `HeatLegend` is shared by hop history and the hourly heatmap: loss uses fixed percent stops, latency/jitter use the displayed data's maximum in milliseconds, and no response/no data have distinct markers.
