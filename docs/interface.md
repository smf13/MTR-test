...
- **Route by place**, under the map, spells the same path out in words: *Berlin monitor › Frankfurt hops 3–4 › London hop 6 › Frankfurt hop 7 › London hop 8, target*. Each step also names the networks it crosses, as the autonomous system number and the organisation behind it (**AS64500 Example Transit GmbH**). Each step is a button: select it to pan the map to that place, open its marker popup and expand a panel listing that step's hops with host names, addresses, networks, latency and loss. On narrow screens the list becomes one step per row, each with a leading marker (a dot for the start, a chevron for every following place), so the rows line up. Select it again to close the panel. Selecting a marker on the map opens the matching step. With a keyboard, use Left/Right or Home/End to move between steps and Enter or Space to open one.
