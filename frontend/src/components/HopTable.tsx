                  ...
                  {noReply ? (
                    ...
                    <div className="leading-tight">
                      <div className="flex items-center gap-1.5">
                        ...
                        {isDst && (
                          <span className="rounded px-1 py-px text-[10px] font-semibold uppercase tracking-wide" style={{ background: "var(--accent)", color: "#fff" }}>
                            dst
                          </span>
                        )}
                      </div>
                      {h.hostname && <div className="font-mono text-[11px] text-faint">{h.ip}</div>}
                    </div>
                  )}
                </td>
                {!compact && <td className="font-mono text-xs text-muted">{h.asn || "–"}</td>}
                <td className="text-right">
                  <span className="inline-block min-w-[3.2rem] rounded px-1.5 py-0.5 text-right text-xs font-semibold" style={{ background: lossColor(h.loss_pct, 0.22), color: h.loss_pct > 0 ? lossColor(h.loss_pct) : "var(--text-muted)" }}>
                    {fmtNum(h.loss_pct, 1)}%
                  </span>
                </td>
